...
MapperFactory(sys::NES &console, cart::Cartridge const &c, vid::Registers &reg,
  ...
  switch (c.mapper) {
  case 0:
    return std::make_unique<NROM>(console, c, reg, oam, pad);
  case 1:
    return std::make_unique<MMC1>(console, c, reg, oam, pad);
  case 2:
    return std::make_unique<UxROM>(console, c, reg, oam, pad);
  case 3:
    return std::make_unique<CNROM>(console, c, reg, oam, pad);
  case 4:
    return std::make_unique<MMC3>(console, c, reg, oam, pad);
  case 9:
    return std::make_unique<MMC2>(console, c, reg, oam, pad);
  case 11:
    return std::make_unique<ColorDreams>(console, c, reg, oam, pad);
  default:
